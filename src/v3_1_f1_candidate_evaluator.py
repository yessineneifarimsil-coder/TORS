...
"""
Pre-specified v3.1-F1 compensated signed-curvature candidate-family evaluator.

This module evaluates the version-frozen v3.1-F1 compensated signed-curvature
family without modifying the production technology response generator.

Selection rule
--------------
Select the smallest tau in the frozen ladder that passes:
1) all D2.8 numerical structural gates,
2) all D2.9 criterion-level LRV/NSV scientific gates,
3) all D2.9 pathway-level SRE reachability gates,
4) all frozen response/invariance gates.

Layer-B winner geometry is computed descriptively and is never used to
choose tau because D2.9 did not activate a numeric T_R_sci.
"""
...
DEVELOPMENT_SEEDS = (27001, 27002, 27003, 27004, 27005)
RESERVED_SEEDS = (28001, 28002, 28003, 28004, 28005)
...
SIGNED_PROFILE_NAMESPACE = 2009
TAU_REFERENCE = 0.0
TAU_UPPER_ANCHOR = 1.0
...
def require_clean_git() -> tuple[str, str]:
    ...
    if status:
        raise RuntimeError(
            "v3.1-F1 REFUSES TO RUN FROM A DIRTY WORKING TREE.\n"
...
def load_frozen_thresholds() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if d29_refs["layer_b"]["activated"] is not False:
        raise RuntimeError(
            "v3.1-F1 evaluator assumes D2.9 Layer-B non-activation."
        )
...
def signed_profile(
    ...
    """
    Build the frozen v3.1-F1 signed alternative-specific profile.

    The complete symmetric grid is permuted once per benchmark instance and
    criterion using SeedSequence([seed, 2009, criterion_number]).
    """
    active_ids = sorted(
...
def _v30_anchor_shape(
    opportunity: np.ndarray | float,
    anchor_p: int,
    v: np.ndarray | float,
) -> np.ndarray:
    """Frozen v3.1 anchor shape, restricted to p=2 or p=3."""
    if anchor_p not in (2, 3):
        raise ValueError("v3.1 anchor_p must be 2 or 3.")
    ...
    a = np.abs(v_arr)
    negative = (1.0 - a) * o + a * np.power(o, anchor_p)
    positive = (
        (1.0 - a) * o
        + a * (1.0 - np.power(1.0 - o, anchor_p))
    )
    values = np.where(
        v_arr < 0.0,
        negative,
        np.where(v_arr > 0.0, positive, o),
    )
    return np.asarray(values, dtype=float)


def _v30_anchor_shape_derivative(
    opportunity: np.ndarray | float,
    anchor_p: int,
    v: np.ndarray | float,
) -> np.ndarray:
    """Analytic derivative of the frozen p=2 or p=3 anchor shape."""
    if anchor_p not in (2, 3):
        raise ValueError("v3.1 anchor_p must be 2 or 3.")
    ...
    negative = (
        (1.0 - a)
        + a * anchor_p * np.power(o, anchor_p - 1)
    )
    positive = (
        (1.0 - a)
        + a * anchor_p * np.power(1.0 - o, anchor_p - 1)
    )
    values = np.where(
        v_arr < 0.0,
        negative,
...
def headroom_shape(
    ...
    """Frozen v3.1 shape: (1-tau)F_2,v + tau F_3,v."""
    tau_value = float(tau)
    if tau_value < -EPS or tau_value > 1.0 + EPS:
        raise ValueError("tau must lie in [0,1].")

    f2 = _v30_anchor_shape(opportunity, 2, v)
    f3 = _v30_anchor_shape(opportunity, 3, v)
    return np.asarray(
        (1.0 - tau_value) * f2 + tau_value * f3,
        dtype=float,
    )
...
def headroom_shape_derivative(
    ...
    """Analytic derivative of the frozen v3.1 interpolated shape."""
    tau_value = float(tau)
    if tau_value < -EPS or tau_value > 1.0 + EPS:
        raise ValueError("tau must lie in [0,1].")

    d2 = _v30_anchor_shape_derivative(opportunity, 2, v)
    d3 = _v30_anchor_shape_derivative(opportunity, 3, v)
    return np.asarray(
        (1.0 - tau_value) * d2 + tau_value * d3,
        dtype=float,
    )
...
def candidate_response(
    ...
    """
    Frozen v3.1-F1 response:

      g = theta*o + (u-theta)*F_{tau,v}(o).
    """
...
def build_candidate_responses(
    ...
    """
    Overwrite only C1-C7 systematic responses with frozen v3.1-F1.
        ...
        for _, alternative in benchmark["alternatives"].items():
            ...
            if capability_class == "0":
                ...
                if np.any(values < -EPS):
                    raise AssertionError(
                        f"{criterion}/{alt_id}: negative v3.1-F1 response."
                    )
    ...
    for col, expected in protected.items():
        if not np.array_equal(
            ...
            raise AssertionError(
                f"{col} changed during v3.1-F1 candidate construction."
            )
...
def build_gate_summary(
    ...
    for tau in TAU_GRID:
        ...
        if len(a) != 7:
            raise AssertionError(
                f"tau={tau}: expected 7 Layer-A criterion rows."
            )
        if len(r) != 12:
            raise AssertionError(
                f"tau={tau}: expected 12 reachability pathway rows."
            )
        if len(b) != 35:
            raise AssertionError(
                f"tau={tau}: expected 35 seed/criterion invariant rows."
            )
        if len(inv) != 5:
            raise AssertionError(
                f"tau={tau}: expected 5 C8-C10 invariance rows."
            )
...
def select_tau(
    ...
    return float(eligible.iloc[0]["tau"])
...
def run(
    ...
    for seed in DEVELOPMENT_SEEDS:
        ...
        if len(
            ...
            raise AssertionError(
                "v3.1-F1 requires exactly 1000 FIT+WEIGHT contexts."
            )
        ...
        if (
            ...
            raise AssertionError(
                "External TEST entered v3.1-F1 evaluation."
            )
        ...
        # tau=0 and tau=1 are descriptive frozen-anchor references only.
        for tau in (
            (TAU_REFERENCE,)
            + TAU_GRID
            + (TAU_UPPER_ANCHOR,)
    ...
    # Candidate selection excludes tau=1.
    ...
    selection = {
        "selected_tau":
            selected,
        "selection_rule":
            "minimum frozen selectable tau passing all numerical, Layer-A D2.9 scientific, reachability D2.9, and invariant gates",
        ...
        "candidate_family":
            "v3.1-F1 interpolated monotone headroom-shape response",
        "family_failed":
            bool(
                selected is None
            ),
    ...
    metadata = {
        ...
        "reserved_seeds_used":
            False,
        "v30_development_seeds_used":
            False,
        "legacy_reserved_seeds_used":
            False,
        "d29_reserved_seeds_used":
            False,
        "reserved_seeds":
            list(RESERVED_SEEDS),
        "structural_validation_seeds":
            list(STRUCTURAL_VALIDATION_SEEDS),
        "v30_development_seeds":
            list(V30_DEVELOPMENT_SEEDS),
        "legacy_reserved_seeds":
            list(LEGACY_RESERVED_SEEDS),
        "d29_reserved_seeds":
            list(D29_RESERVED_SEEDS),
        "primary_seeds":
            list(PRIMARY_SEEDS),
    ...
    summary_lines = [
        "v3.1-F1 MONOTONE HEADROOM-SHAPE CANDIDATE EVALUATION",
        f"git_commit: {commit}",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate version-frozen v3.1-F1 "
            "interpolated monotone headroom-shape candidate grid."
        )
    )
    parser.add_argument(
        "--output-dir",
