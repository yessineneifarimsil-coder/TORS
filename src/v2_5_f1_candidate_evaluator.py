...
"""
Pre-specified v2.5-F1 compensated signed-curvature candidate-family evaluator.

This module evaluates the version-frozen v2.5-F1 compensated signed-curvature
...
def require_clean_git() -> tuple[str, str]:
    ...
    if status:
        raise RuntimeError(
            "v2.5-F1 REFUSES TO RUN FROM A DIRTY WORKING TREE.\n"
...
def load_frozen_thresholds() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if d27_refs["layer_b"]["activated"] is not False:
        raise RuntimeError(
            "v2.5-F1 evaluator assumes D2.7 Layer-B non-activation."
        )
...
def signed_profile(
    ...
    """
    Build the frozen v2.5-F1 signed alternative-specific profile.

    The complete symmetric grid is permuted once per benchmark instance and
    criterion using SeedSequence([seed, 2006, criterion_number]).
    """
    active_ids = sorted(
...
def candidate_response(
    ...
    """
    Frozen terminal v2.5-F1 ceiling-scaled signed-curvature response:

      theta*o + eta*[(u-theta)*o + u*v*o*(1-o)].
    """
...
def build_candidate_responses(
    ...
    """
    Overwrite only C1-C7 systematic responses with frozen terminal v2.5-F1.
    ...
    for criterion in C8_C10:
        col = f"g_{criterion}"
        protected[col] = baseline[col].to_numpy(dtype=float, copy=True)
        ...
        for _, alternative in benchmark["alternatives"].items():
            ...
            if capability_class == "0":
                ...
                if np.any(values < -EPS):
                    raise AssertionError(
                        f"{criterion}/{alt_id}: negative v2.5-F1 response."
                    )
    ...
    for col, expected in protected.items():
        got = result[col].to_numpy(dtype=float)
        if not np.array_equal(got, expected):
            raise AssertionError(
                f"{col} changed during v2.5-F1 candidate construction."
            )
...
def run(
    ...
    for seed in DESIGN_SEEDS:
        ...
        if len(
            ...
            raise AssertionError(
                "v2.5-F1 requires exactly 1000 FIT+WEIGHT contexts."
            )
        ...
        if (
            ...
            raise AssertionError(
                "External TEST entered v2.5-F1 evaluation."
            )
    ...
    selection = {
        ...
        "candidate_family":
            "v2.5-F1 ceiling-scaled signed-curvature response",
        "family_failed":
            bool(
                selected is None
            ),
    ...
    summary_lines = [
        "v2.5-F1 CEILING-SCALED SIGNED-CURVATURE CANDIDATE EVALUATION",
        f"git_commit: {commit}",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate version-frozen v2.5-F1 "
            "compensated signed-curvature candidate ladder."
        )
    )
    parser.add_argument(
        "--output-dir",
